...
> - `ab` 与 `prompts` 两模块按"不编造结果"原则：环境不满足（未装 Ollama / Key 无效）时执行脚本走 dry-run 只写待跑清单，README 与报告中如实标注状态；工具链（题集、模板、判定代码、分析脚本）全部就绪，补齐环境后一条命令生成真实数据。
...
4. **模型对比口径受限**：A/B 中的 Qwen 取 1.5b 蒸馏小规格（本机算力限制），不代表 Qwen 系列上限；judge 分来自与被评方同源的 LLM，存在自我偏好偏差，需人工复核；当前环境 A/B 与 Prompt 复测结果处于待跑状态（工具链就绪，README 如实标注）。
